...
void ComputerPlayer::generateMoves()
{
	...
	bestMoveScore = 0;
	int moveScore;
	bool canMove, rotate = false, counterClock = false, res = false;
...
bool ComputerPlayer::moveShape2()
{
	...
	{
		if (bestMove[0] == 0)
			return shape.move(GameConfig::eKeys::LEFT, index);
		else
			return shape.move(GameConfig::eKeys::RIGHT, index);
	}
	while (bestMove[3] != 0)
	{
		if (bestMove[4] == 0)
			return shape.move(GameConfig::eKeys::ROTATE_CLOCKWISE, index);
		else
			return shape.move(GameConfig::eKeys::ROTATE_COUNTER_CLOCKWISE, index);
	}
	while (bestMove[2] != 0)
	{
		if (bestMove[0] == 0)
